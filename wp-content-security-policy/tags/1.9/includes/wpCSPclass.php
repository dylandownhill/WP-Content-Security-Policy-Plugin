...
class wpCSPclass extends WP_REST_Controller{
	...
	const HEADERTAG_ENFORCEPOLICY = 'Content-Security-Policy' ;
	...
	public static function add_header() {
		...
		$CSPHeaderTag = !empty( $ReportOnly ) ? self::HEADERTAG_REPORTONLY : self::HEADERTAG_ENFORCEPOLICY;
		...
		if ( $LogViolations === true || !empty( $ReportOnly ) ) {
			$Nonce =  wp_create_nonce( "wp_rest" );
			$ReportURIBase = site_url( "/wp-json/" . wpCSPclass::ROUTE_NAMESPACE . "/" . wpCSPclass::ROUTE_BASE . "/LogPolicyViolation" ) ;
			$URLParams = array("_wpnonce" => $Nonce);
			$ReportURI = add_query_arg( $URLParams, $ReportURIBase) ;
			$CSPOutput[] = "report-uri " . $ReportURI  ;
		}
		// Output the CSP header
		header($CSPHeaderTag . ": " . implode( "; ", $CSPOutput ));
	}
