...
class wpCSPAdmin extends WP_REST_Controller{
	...
	public static function options_page() {
		...
		foreach( wpCSPclass::$CSP_Policies as $PolicyKey => $CSPPolicy) :
			...
			if ( !empty( $Errors )) {
				$ErrorOutput[] = "<tr><td><a href='#anchor". $PolicyKey ."'>".$PolicyKey."</a></td><td><ul><li>". implode("</li><li>",$Errors) . "</li></ul></td></tr>" ;
			}
		endforeach;		
		?>
     <div class="wrap">
	                    ...
	                    <div id="post-body-content">
	                         ...
	                         <?php settings_fields( wpCSPclass::SETTINGS_OPTIONS_SECTION ); // Outputs nonces and other necessary items?>
	                              <table class="form-table">
	                                   ...
	                                   <tr class='wpcsp_option_row'><th scope="row"><?php _e( "Mixed Content", 'wpcsp' ); ?></th>
	                                        <td class='wpcsp_option_cell'>
	                                   ...
	                                   <tr><th scope="row"><?php _e( "Policy Entries", 'wpcsp' ); ?></th>
	                                   	<td><p>Content Security Policy allows the following entries - one per line:</p>
		                                   	...
		                                   	<tr><td>'strict-dynamic'</td><td>The trust explicitly given to a script present in the markup, by accompanying it with a nonce or a hash, shall be propagated to all the scripts loaded by that root script.</td></tr>
		...
		if( is_array( $Policies)){
			foreach( $Policies as $Policy ) {
				...
				if ( $StrippedPolicy == 'strict-dynamic' && $Policy != "'strict-dynamic'") {
					$return[] = "Entry for <strong>strict-dynamic</strong> should read <strong>'strict-dynamic'</strong> (with single quotes) - entry: " .$Policy ;
				}
